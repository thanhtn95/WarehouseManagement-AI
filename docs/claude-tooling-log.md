...
## Login spec refined: forced password change on first login (2026-09-15)

Triggered by: "let refine the login specs a little bit, first time login
after account created will trigger force password change right away
before you could login normally next time."

| Tool | Role |
|---|---|
| Grep + Read | Checked what the design document already said before designing anything, per this project's own established pattern — found the *only* existing "forced credential change" content was scoped narrowly to the first administrator during Phase 4 fleet provisioning (§6.20 step 4, duplicated in §11.2), not a general rule for every newly created account. Also found `POST /users`'s own prose already named "a future credential-reset endpoint" as an anticipated, unbuilt gap — a direct anchor point for this request rather than a fresh invention. |
| Plain conversation (no tool) | One scope call made without being asked, stated plainly rather than silently assumed: restricted the forced-change behaviour to `password` credentials (staff) only, excluding `pin`/`badge` (operator) — the user's own wording said "password," and operator PIN/badge are admin-issued for repeated shared-device access rather than a personal secret the holder would expect to privately rotate, so extending the same flow there would be a different design decision than what was asked. Reused an existing idiom rather than inventing a new one: the new `changeToken` on `403 …/credential-change-required` is the same single-use, short-lived-grant shape as `POST /auth/elevate`'s `grantToken`, so the API doesn't gain a second pattern for "prove you still hold the credential, get a narrow one-shot capability." Distinguished this first-login case explicitly from admin-initiated credential resets on an *existing* account, which stays a separate, still-open item — not resolved by this change, and said so in the doc rather than letting the two blur together. |
| Edit | `wms-design-document.md`: added `credential.must_change` (§2.1); added the `403 .../credential-change-required` response to `POST /auth/staff/login` and a new `POST /auth/credential-change` endpoint (§5.1); added the matching row to the Error Catalogue (§7.1); added a cross-referencing note under `POST /users` (§5.11); reworded the Phase-4 provisioning flow's "first administrator" step (§6.20, §11.2) so it reads as an instance of the general rule rather than a provisioning-specific behaviour. `wms-screen-inventory.md`: broadened the "Forced credential change" screen's purpose/primary-user from "System Administrator, first login only" to any staff account's first login, and filled in its previously-blank endpoint column now that one exists. |

## Propagating the login-spec change and the Phase 1A completeness gaps (2026-09-15)

Triggered by: "update the documents and phases plans" — ambiguous enough
between two live threads (the login-spec refinement two turns back, and
the Phase 1A backend-completeness audit before that) that an
`AskUserQuestion` was used rather than guessed; the user picked both.

| Tool | Role |
|---|---|
| `AskUserQuestion` | Offered the two readings plus "something else"; the user selected both rather than one, resolving the ambiguity precisely instead of guessing which thread "the documents" meant. |
| Grep + Bash | For the propagation half: confirmed `wms-project-proposal.md`'s Auth scope row was the only stale copy of the pre-refinement login description (no other doc echoes it). For the gap half: re-verified the earlier audit's findings against fresh `grep`/`Bash` output rather than trusting the prior turn's report from memory — re-ran the `INSERT INTO warehouse/zone/device` search across test files to name the exact tests as evidence in the shortcuts entries, since a shortcut entry that cites no concrete evidence is exactly the kind of unfalsifiable claim this file's own discipline is supposed to prevent. |
| Plain conversation (no tool) | The judgment call on *where* each finding belongs, since shortcuts.md's own definition is specific — "hardcoded, stubbed, or seeded... that will need replacing," not simply "not yet built." Test files inserting master data, devices, and (implicitly) roles directly via SQL because no other path exists is a real workaround-standing-in-for-a-feature, so those five gaps (master data CRUD, device CRUD, custom roles, audit log read, elevate/exit-criterion-6) went into `shortcuts.md`. The softer findings — exit criterion 4 tested at 8 operators instead of 30, criterion 7 unmeasurable until a real deployment exists — are test-rigor/measurement gaps against an already-written criterion, not a corner cut standing in for a feature, so those went into the proposal's own status snapshot instead, not forced into shortcuts.md just to have a single tracking location. |
| Edit | `wms-project-proposal.md`: added "(forced password change on first login)" to the Auth scope row; added a **Status as of 2026-09-15** subsection directly under the Phase 1A exit-criteria list, explicitly labeled as a live-tracked exception to the document's own plan-not-status nature — same pattern as the architecture review's Consolidated Open Decisions table, cited as precedent in the new text itself so a future session doesn't have to rediscover why an exception exists here. `docs/shortcuts.md`: six new `[1A]` Open entries (master data CRUD, device CRUD, custom role composition, auth audit log read, the elevate/exit-criterion-6 gap, the missing allocation-concurrency test), each citing the specific test files or proposal clause that evidences it rather than a bare assertion. |
